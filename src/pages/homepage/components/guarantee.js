import React from 'react';

export default function GuaranteeSection() {
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-10 px-2 py-24 sm:px-6 lg:flex-row lg:px-8">
      <div className="flex flex-col items-center gap-3 lg:w-1/3">
        <div className="">
          <img
            className="h-[50px] w-[50px]"
            alt="icon protected money"
            src="data:image/png;base64,"
          />
        </div>
        <h2 className="font-subheading text-2xl">Booking Protection Guarantee</h2>
        <p className="text-center font-body text-base font-light">
          In the unlikely event a Host needs to cancel your booking within 30 days of check-in,
          we’ll find you a similar or better home, or we’ll refund you.
        </p>
      </div>
      <div className="flex flex-col items-center gap-3 lg:w-1/3">
        <div className="">
          <img
            className="h-[50px] w-[50px]"
            alt="icon shield"
            src="data:image/png;base64,"
          />
        </div>
        <h2 className="font-subheading text-2xl">Get-What-You-Booked</h2>
        <p className="text-center font-body text-base font-light">
          If at any time during your stay you find your listing isnt as advertised
        </p>
      </div>

      <div className="flex flex-col items-center gap-3 lg:w-1/3">
        <div className="">
          <img
            className="h-[50px] w-[50px]"
            alt="icon luggage"
            src="data:image/png;base64,"
          />
        </div>
        <h2 className="font-subheading text-2xl">Check-In Guarantee</h2>
        <p className="text-center font-body text-base font-light">
          If you can’t check into your home and the Host cannot resolve the issue, we’ll find you a
          similar or better home for the length of your original stay, or we’ll refund you.
        </p>
      </div>
    </div>
  );
}
